...
def sort_by_cost_heuristic(connections: dict) -> list[tuple[tuple, int]]:
    return sorted(connections.items(), key=lambda x: x[1] + nodes[x[-1]].heuristic)


def sort_by_cost(connections: dict) -> list[tuple[tuple, int]]:
